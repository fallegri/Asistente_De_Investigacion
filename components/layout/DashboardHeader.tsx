...
// Recibe nombre y email como props del Server Component padre.
// Sin hooks de auth — sin riesgo de hidratación.
...
export function DashboardHeader({ nombre, email }: DashboardHeaderProps) {
  ...
  const displayName = nombre.split(' ')[0] || email.split('@')[0]
    ...
    <header className="bg-white border-b border-[var(--color-border)] sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        <Link href="/dashboard" className="flex items-center gap-2 group">
          ...
          <span className="hidden sm:block text-sm font-bold text-stone-700">
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 pl-2 pr-2 sm:pr-3 py-1.5 rounded-xl hover:bg-[var(--color-surface-2)] transition-colors"
          >
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-52 bg-white border border-[var(--color-border)] rounded-xl shadow-lg py-1.5 z-50 animate-fade-in-up">
              <div className="px-3.5 py-2.5 border-b border-[var(--color-border)]">
                <p className="text-xs font-semibold text-[var(--color-text-primary)] truncate">{nombre || email}</p>
                <p className="text-[10px] text-[var(--color-text-muted)] truncate mt-0.5">{email}</p>
              </div>
              <div className="py-1">
                <button
